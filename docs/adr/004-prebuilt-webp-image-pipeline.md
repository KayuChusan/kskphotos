...
kskphotos は写真ポートフォリオサイトなので、1ページに高解像度の写真が何枚も並びます。撮影機材は Sony α7R IV（約6100万画素）で、RAW（ARW）→ Lightroom 現像 → JPEG 書き出しという流れのため、元画像は非常に大きいものです。これを「速く・安く」配信する仕組みが必要でした。
