      ...
      ? [
          {
            ...
            body: (
              <ol className="list-decimal space-y-0.5 pl-5">
                ...
                <li>
                  本契約が終了（完了・解約・解除）した場合、乙は甲に対し、
                  {f.copyright === "譲渡"
                    ? "本サイトのソースコード（開発用リポジトリ）・デザインデータ（制作用ファイル）を含む一式、コンテンツおよびサイトが保有するデータ、ドメイン・サーバーの管理情報を引き渡す。"
                    : "本サイトを稼働可能な状態（公開済みのファイル一式）、コンテンツおよびサイトが保有するデータ、ドメイン・サーバーの管理情報、甲名義の各種設定を引き渡す。ソースコード（開発用リポジトリ）およびデザインデータ（制作用ファイル）は乙に著作権が留保されるため引き渡しの対象外とし、これらを希望する場合は別途協議（著作権の譲渡・追加費用）による。"}
                  引き渡しに相当の作業を要する場合の費用・方法は別途協議する。
                </li>
                <li>本サイトが保有する利用者の個人情報（問い合わせ・会員情報等）は甲に引き渡し、引き渡し後に乙が保有する複製は速やかに消去する。</li>
